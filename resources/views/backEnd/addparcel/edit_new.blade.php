                                     ...
                                     <div class="row">
										...
										<div class="col-sm-6">
											...
											<select type="text"  class="orderType form-control{{ $errors->has('orderType') ? ' is-invalid' : '' }}"  name="orderType" required="required">
											    ...
											    <option value="{{$value->id}}" data-deliverycharge="{{$value->deliverycharge}}" extracharge="{{$value->extradeliverycharge}}" cod="{{$value->cod}}" @if($value->id == $edit_data->orderType) selected @endif>{{$value->title}}</option>
											    @endforeach
											</select>    
										...
										<div class="col-sm-6">
											...
											<select type="text"  class="reciveZone select2 form-control{{ $errors->has('reciveZone') ? ' is-invalid' : '' }}" value="{{ old('reciveZone') }}" name="reciveZone" id="reciveZone" placeholder="Delivery Area" required="required">
											    <option value="{{ $area->id }}" selected="selected">{{ $area->zonename }}</option>
        					    ...
        					    <div class="parcel-details-instance">
            						...
            						<div class="content calculate_result">
            							<div class="row">
            								...
            								<div class="col-sm-4">
            									<p><span class="cashCollection">{{ $edit_data->cod }}</span>  N</p>
            								</div>
            							</div>
            							<!-- row end -->
            							<div class="row">
            								...
            								<div class="col-sm-4">
            									<p><span class="devlieryCharge">{{ $edit_data->deliveryCharge }}</span> N</p>
            								</div>
            							</div>
            							<!-- row end -->
            							<div class="row">
            								...
            								<div class="col-sm-4">
            									<p><span class="codCharge">{{ $edit_data->codCharge }}</span> N</p>
            								</div>
            							</div>
            							<hr>
            							<!-- row end -->
            							<div class="row total-bar">
            								...
            								<div class="col-sm-4">
            									<p><span class="total">{{ ($edit_data->deliveryCharge + $edit_data->codCharge - $edit_data->cod) * (-1) }}</span> N</p>
            								</div>
            							</div>
            							<!-- row end -->
            							<div class="row">
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$("input[name='cod']").on("keyup", function() {
		    $(".cashCollection").text($("input[name='cod']").val());
		    codUpdate();
		    devlieryCharge();
		})
		...
		function codUpdate() {
		    ...
		    var charge = $('select[name="orderType"] option:selected').attr("cod");
		    charge = charge ? parseInt(cash) * (parseInt(charge) / 100) : 0;
		    $(".codCharge").text(charge);
		    $("#codCharge").val(charge);
		}
		
		function devlieryCharge() {
		    var stateCharge = $('select[name="orderType"] option:selected').attr('data-deliverycharge');
		    var extraCharge = $('select[name="orderType"] option:selected').attr('extracharge');
		    var cod = $('select[name="orderType"] option:selected').attr('cod');
