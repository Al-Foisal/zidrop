...
<body>
          ...
          <div class="short_button">
            <button onclick="goBack()" style="color: #135d30;border: 0;padding: 6px 12px;margin-bottom: 8px !important;display: block;text-align: center;background: 0;border-radius: 5px;"><i class="fa fa-arrow-circle-left"></i> Back</button> 
          </div>
        ...
        <table class="table table-bordered parcel-invoice">
          <tbody>
            ...
            <tr class="item">
                ...
                <td><center>{{$value->cod}}</center></td>
                <td> <center>{{$value->deliveryCharge+$value->codCharge}}</center></td>
                
                <!--<td> {{$value->cod-($value->deliveryCharge+$value->codCharge)}}</td>-->
                <td><center>{{$value->cod-($value->deliveryCharge+$value->codCharge)}} /-</center></td>
            </tr>
            
            @php
              $totalt += $value->cod;
              $totalc += $value->deliveryCharge+$value->codCharge;
            ...
            <tr class="heading">
                <td></td>
                <td></td>
                <td><P align="right">Sub Total:</P></td>
                <td>{{$totalt}} /-</td>
                <td>Total: {{$totalc}} /-</td>
                <td>Total: {{$total}} /-</td>
            </tr>
...
Zuri Express Charge: Tk {{$totalc}} /-
<br>
Total Merchant Payment Amount: Tk {{$total}} /-
<br>
...
<textarea id="w3review" name="w3review" rows="1" cols="102"></textarea>
</form>
</body>
</html>
